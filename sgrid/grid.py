...
class SeleniumGrid(RequestBase):
    ...
    def compose_call(self, command):
        cmd = "{compose_binary} -f {compose_file} {command}".format(
            compose_binary=self.compose_binary,
            compose_file=self._compose_file,
            command=command,
        )
        return subprocess.check_output(cmd.split(" "))

    def start_grid(self, num_nodes=None):
        num_nodes = num_nodes or self.num_nodes
        print("Starting grid with %s nodes" % num_nodes)
        self.compose_call(
            "up -d --scale chrome={num_nodes}".format(num_nodes=num_nodes)
        )
        ping_service(url=self.HUB_URL)
        ping_service(
            url=self.API_URL,
            condition=lambda resp: int(resp.json()["slotCounts"]["total"]) == num_nodes,
        )
